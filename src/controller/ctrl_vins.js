...
// Définir la fonction qui utilisera la méthode GET pour récupérer les éléments du tableau vins
exports.getData_vins = (request,response) =>{
...
exports.getDataByName = (request, response) => {
    ...
    fs.readFile("./src/model/carte.json" , (err, data) => {
        ...
        if (err) {
            ...
            const data_name = manip_data.vins.find(
...
exports.getDataById = (request, response) =>{
    ...
    fs.readFile("./src/model/carte.json", (err, data) =>{
        ...
        if (err) {
            ...
            const data_id = manip_data.vins.find(
...
exports.createData = (request, response) =>{
    ...
    // Condition erreur 500
        ...
        // sinon pas d'erreur
            ...
            // Définir la longueur du tableau vins
            let arrayLength = existing_data.vins.length;
                // Si le tableau n'est pas vide
                if (arrayLength > 1) {
                    // Récupère le dernier objet du tableau vins
                    let lastObject = existing_data.vins[arrayLength - 1];
                    // Créer et insérer la new data avec l'id = à celle du dernier objet +1
                    existing_data.vins.push({
                        "id": lastObject.id + 1, 
...
exports.deleteData = (request,response)  =>{
    ...
    if (err) {
        ...
        if (!data_id) {
            ...
            existing_data.vins = existing_data.vins.filter(
                (obj) => obj.id != parseInt(request.params.id));
