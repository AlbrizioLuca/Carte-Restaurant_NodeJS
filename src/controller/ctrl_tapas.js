...
// Définir la fonction qui utilisera la méthode GET pour récupérer les éléments du tableau tapas
exports.getData_tapas = (request,response) =>{
    ...
    fs.readFile("./src/model/carte.json", (err,data) =>{
        ...
        if (err){
            ...
            response.status(200).json(JSON.parse(data).tapas)
        }
    })
}
...
exports.getDataByName = (request, response) => {
    ...
    fs.readFile("./src/model/carte.json" , (err, data) => {
        ...
        if (err) {
            ...
            const data_name = manip_data.tapas.find(
...
exports.getDataById = (request, response) =>{
    ...
    fs.readFile("./src/model/carte.json", (err, data) =>{
        ...
        if (err) {
            ...
            const data_id = manip_data.tapas.find(
...
exports.createData = (request, response) =>{
    ...
    // Condition erreur 500
        ...
        // sinon pas d'erreur
            ...
            // Définir la longueur du tableau tapas
            let arrayLength = existing_data.tapas.length;
                // Si le tableau n'est pas vide
                if (arrayLength > 1) {
                    // Récupère le dernier objet du tableau tapas
                    let lastObject = existing_data.tapas[arrayLength - 1];
                    // Créer et insérer la new data avec l'id = à celle du dernier objet +1
                    existing_data.tapas.push({
                        "id": lastObject.id + 1, 
...
exports.updateData = (request, response) =>{
    ...
    fs.readFile("./src/model/carte.json", (err,data)=>{
        ...
        if (err) {
            ...
            const data_id = existing_data.tapas.find(
            (obj) => obj.id === parseInt(request.params.id)
            );
...
exports.deleteData = (request,response)  =>{
    ...
    if (err) {
        ...
        if (!data_id) {
            ...
            existing_data.tapas = existing_data.tapas.filter(
                (obj) => obj.id != parseInt(request.params.id));
