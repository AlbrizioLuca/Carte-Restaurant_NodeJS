...
// Définir la fonction qui utilisera la méthode GET pour récupérer les éléments du tableau desserts
exports.getData_desserts = (request,response) =>{
...
exports.getDataByName = (request, response) => {
    ...
    fs.readFile("./src/model/carte.json" , (err, data) => {
        ...
        if (err) {
            ...
            const data_name = manip_data.desserts.find(
...
exports.getDataById = (request, response) =>{
    ...
    fs.readFile("./src/model/carte.json", (err, data) =>{
        ...
        if (err) {
            ...
            const data_id = manip_data.desserts.find(
...
exports.createData = (request, response) =>{
    ...
    // Condition erreur 500
        ...
        // sinon pas d'erreur
            ...
            // Définir la longueur du tableau desserts
            let arrayLength = existing_data.desserts.length;
                // Si le tableau n'est pas vide
                if (arrayLength > 1) {
                    // Récupère le dernier objet du tableau desserts
                    let lastObject = existing_data.desserts[arrayLength - 1];
                    // Créer et insérer la new data avec l'id = à celle du dernier objet +1
                    existing_data.desserts.push({
                        "id": lastObject.id + 1, 
...
exports.updateData = (request, response) =>{
    ...
    fs.readFile("./src/model/carte.json", (err,data)=>{
        ...
        if (err) {
            ...
            const data_id = existing_data.desserts.find(
            (obj) => obj.id === parseInt(request.params.id)
            );
...
exports.deleteData = (request,response)  =>{
    ...
    if (err) {
        ...
        if (!data_id) {
            ...
            existing_data.desserts = existing_data.desserts.filter(
                (obj) => obj.id != parseInt(request.params.id));
