...
// Définir la fonction qui utilisera la méthode GET pour récupérer les éléments du tableau plats
exports.getData_plats = (request,response) =>{
...
exports.getDataByName = (request, response) => {
    ...
    fs.readFile("./src/model/carte.json" , (err, data) => {
        ...
        if (err) {
            ...
            const data_name = manip_data.plats.find(
...
exports.getDataById = (request, response) =>{
    ...
    fs.readFile("./src/model/carte.json", (err, data) =>{
        ...
        if (err) {
            ...
            const data_id = manip_data.plats.find(
...
exports.createData = (request, response) =>{
    ...
    // Condition erreur 500
        ...
        // sinon pas d'erreur
            ...
            // Définir la longueur du tableau plats
            let arrayLength = existing_data.plats.length;
                // Si le tableau n'est pas vide
                if (arrayLength > 1) {
                    // Récupère le dernier objet du tableau plats
                    let lastObject = existing_data.plats[arrayLength - 1];
                    // Créer et insérer la new data avec l'id = à celle du dernier objet +1
                    existing_data.plats.push({
                        "id": lastObject.id + 1, 
...
exports.updateData = (request, response) =>{
    ...
    fs.readFile("./src/model/carte.json", (err,data)=>{
        ...
        if (err) {
            ...
            const data_id = existing_data.plats.find(
            (obj) => obj.id === parseInt(request.params.id)
            );
...
exports.deleteData = (request,response)  =>{
    ...
    if (err) {
        ...
        if (!data_id) {
            ...
            existing_data.plats = existing_data.plats.filter(
                (obj) => obj.id != parseInt(request.params.id));
